...
```{r}
sweetwhite <- ts(aa$sweetw, frequency = 6, start = c(1980, 1), end = c(1997, 6)) 
mod_hw_6 <- HoltWinters(sweetwhite, seasonal = "mult")
plot(mod_hw_6)
acf(resid(mod_hw_6))
```
...
We note hear that the Holt-Winters exponential smoothing procedure produced estimates for three parameters; for $\alpha$, $\beta$ and $\gamma$, as well as 14 coefficients that will be explained later. The model is:
...
ss1pe(c(0.3633865, 0, 0.4975705), 12)
```
...
b. Give an explanation for why $\alpha$ is so large.

...
In Chapter \@ref(regression), we modelled trends and seasonality of various time series. The residuals left over after modelling the trend and seasonality then need to be modelled in order to get a satisfactory final model. This leads us to the concept of stationary models. 
...
a. Plot the data versus time. Does there seem to be a trend or seasonality to consider? If so, remove the estimated trend using 
b. Examine the `acf` and `pacf` plots and indicate whether a moving average model is appropriate for this data. (Note: you will have to remove the missing data at the beginning of the time series.)
:::

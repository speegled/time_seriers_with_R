...
alpha2 <- .4
eps <- w[1:2]
for(i in 3:1000) eps[i] <- w[i] * sqrt(alpha + alpha1 * eps[i - 1]^2 + alpha2 * eps[i - 2]^2)
plot(eps, type = "l")
acf(eps)
acf(eps^2)
```
...
for(x  in 1:nrow(orders)) {
  orders$AIC[x] <- tryCatch(AIC(garch(eps[101:5000], order = as.numeric(orders[x,]), 
...
Model the residuals using a GARCH model. Looks like we need a GARCH(3, 1) model for the residuals.
...
arrange(orders, AIC)
mod2 <- garch(residuals(mod), order = c(3, 1), control = garch.control(trace = FALSE))
summary(mod2)
```
